...
WHERE f.name LIKE ('Tennis Court%') AND m.surname != 'GUEST'
...
SELECT f.name AS facility_name, CONCAT(m.firstname,' ',m.surname) as member_name, 
	CASE WHEN b.memid=0 THEN b.slots*f.guestcost
		ELSE b.slots*membercost END AS booking_cost
FROM Bookings as b
...
SELECT f.name AS facility_name, m.member_name,
	CASE WHEN b.memid=0 THEN b.slots*f.guestcost
	ELSE b.slots*membercost END AS booking_cost
FROM Bookings as b
LEFT JOIN Facilities as f on f.facid = b.facid
LEFT JOIN 
	( SELECT memid, CONCAT(firstname,' ',surname) as member_name from Members) as m
ON b.memid = m.memid
WHERE SUBSTRING(starttime,1,10)='2012-09-14' AND
	CASE WHEN b.memid=0 THEN b.slots*f.guestcost
	ELSE b.slots*membercost END > 30
